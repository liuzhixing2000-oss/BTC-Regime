...
def download_binance_flow(start,end):
    ...
    x=x.rename(columns={"volume":"binance_volume"})
    return x.drop_duplicates("timestamp").set_index("timestamp").sort_index()


def build_conditions(prices,cfg,flow):
    x=build_scores(prices,cfg).join(flow[["binance_volume","taker_buy_volume","trades"]],how="left")
    # Both numerator and denominator must come from the same exchange.
    x["taker_imbalance"]=2*x.taker_buy_volume/x.binance_volume.replace(0,np.nan)-1
